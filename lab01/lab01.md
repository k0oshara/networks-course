...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - <!-- todo --> ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7
   - <!-- todo --> User-Agent, Accept, Accept-Encoding, Host, Connection, Upgrade-Insecure-Requests
...
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - <!-- todo --> да: If-Modified-Since: Tue, 28 Oct  2025 05:59:01 GMT
...
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - <!-- todo --> параллельно: браузер отправляет несколько GET к разным серверам практически сразу после получения html, не дожидаясь завершения загрузки предыдущего объекта.
