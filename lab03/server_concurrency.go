...
const (
    ...
    workerResponseDelay  = 5 * time.Second
)
...
func main() {
    if len(os.Args) != 3 {
        fmt.Fprintf(os.Stderr, "usage: %s server_port concurrency_level\n", filepath.Base(os.Args[0]))
        os.Exit(1)
    }
    ...
    if err != nil || concurrencyLevel < 1 {
        fmt.Fprintln(os.Stderr, "concurrency_level must be a positive integer")
        os.Exit(1)
    }
    ...
    workerSlots := make(chan struct{}, concurrencyLevel)

    log.Printf("dispatcher is listening on http://127.0.0.1:%d with concurrency_level=%d", port, concurrencyLevel)
    ...
    for {
        ...
        go handleInitialConnection(conn, workerSlots)
    }
}

func handleInitialConnection(conn net.Conn, workerSlots chan struct{}) {
    defer conn.Close()
    ...
    log.Printf("dispatcher: request from %s for %s is waiting for a free worker slot", remoteAddr, request.Target)
    workerSlots <- struct{}{}
    log.Printf("dispatcher: worker slot acquired for %s, active workers: %d/%d", request.Target, len(workerSlots), cap(workerSlots))
    ...
    go serveWorkerRequest(workerListener, request, workerSlots)
    ...
    log.Printf("dispatcher: request for %s redirected to worker port %d", request.Target, workerAddr.Port)
    writeResponseWithHeaders(conn, "302 Found", "text/plain; charset=utf-8", []byte("Redirecting to worker port\n"), headers)
}

func serveWorkerRequest(listener net.Listener, initialRequest httpRequest, workerSlots chan struct{}) {
    defer func() {
        <-workerSlots
        log.Printf("worker: finished %s, active workers: %d/%d", initialRequest.Target, len(workerSlots), cap(workerSlots))
    }()
    defer listener.Close()
    ...
    log.Printf("worker: started processing %s from %s on %s", initialRequest.Target, conn.RemoteAddr().String(), listener.Addr().String())
    ...
    time.Sleep(workerResponseDelay)
