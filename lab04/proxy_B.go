...
func main() {
    ...
    blacklistPath := flag.String("blacklist", "blacklist.json", "path to blacklist config file")
    flag.Parse()
...
func (p *proxyServer) serveProxy(w http.ResponseWriter, r *http.Request) {
    ...
    if p.isBlocked(targetURL) {
        p.respondBlocked(w, r.Method, targetURL.String())
        return
    }
...
func loadBlacklist(path string) (*blacklistConfig, error) {
    ...
    for i, domain := range cfg.Domains {
        cfg.Domains[i] = normalizeDomain(domain)
    }
    for i, rawURL := range cfg.URLs {
        cfg.URLs[i] = strings.TrimSpace(rawURL)
    }
...
func (p *proxyServer) isBlocked(targetURL *url.URL) bool {
    if p.blacklist == nil {
        return false
    }

    rawURL := targetURL.String()
    for _, blockedURL := range p.blacklist.URLs {
        if blockedURL == rawURL {
            return true
        }
    }

    host := normalizeDomain(targetURL.Hostname())
    for _, blockedDomain := range p.blacklist.Domains {
...
func (p *proxyServer) respondBlocked(w http.ResponseWriter, method, rawURL string) {
    http.Error(w, "Access denied: this page is blocked by proxy blacklist", http.StatusForbidden)
    p.logger.Printf("%s %s -> %d blocked_by_blacklist", method, rawURL, http.StatusForbidden)
}
