...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   - Не меняются: Source IP Address, Destination IP Address, Protocol, Differentiated Services Field / Type of Service, Total Length, Version, Header Length, Flags, Fragment Offset.

   - Должны оставаться неизменными: Source IP Address, Destination IP Address, Protocol, Differentiated Services Field / Type of Service, Version, Header Length, Total Length, Flags, Fragment Offset.
   ...
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - увеличивается на 1 с каждым следующим ICMP Echo Request
      - Да, закономерность есть: каждое следующее значение Identification больше предыдущего на 1.

5. Какое значение содержится в поле Identification (Идентификация), а какое – в поле TTL
   (выберите какой-либо один пакет ICMP)?

   - Identification = 0xd3e7 (54247)
   - TTL = 1
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - В ICMP Time Exceeded от первого маршрутизатора 192.168.7.1 значение TTL остается одинаковым и равно 64. Значение Identification изменяется от пакета к пакету: 0xce5b, 0xce5c, 0xce5d.
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами?
      ...
      - Да, ICMP Echo Request был фрагментирован. Было создано 3 IP-фрагмента.
      ![alt text](image-7.png)

   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - В разных фрагментах изменяются поля Total Length, Flags / More Fragments, Fragment Offset и Header Checksum.
