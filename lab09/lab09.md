...
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - Потому что ICMP не является транспортным протоколом.
...
#### Вопросы
`traceroute -4 -I -q 3 www.stanford.edu`
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - Почти не отличается: это такой же ICMP Echo Request с Type = 8 и Code = 0. Отличие в том, что traceroute отправляет такие пакеты с постепенно увеличивающимся TTL, а обычный ping использует обычное начальное значение TTL.
...
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - В дополнительных полях содержится часть исходного пакета, вызвавшего ошибку: оригинальный IP-заголовок и начало ICMP Echo Request. Благодаря этому видно, какой пакет был отброшен: его адрес источника, адрес назначения, TTL, протокол, Identifier и Sequence Number.
...
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - Три последних ICMP-пакета, полученных исходным хостом, отличаются от ICMP-пакетов с ошибками тем, что это не сообщения Time Exceeded, а обычные Echo Reply от конечного хоста.
   - Это объясняется тем, что на последних шагах traceroute значение TTL стало достаточно большим, чтобы пакет дошёл до конечного хоста. Поэтому маршрутизаторы уже не отбрасывают пакет из-за истечения TTL, а конечный хост отвечает обычным ICMP Echo Reply.
...
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - Да, такой канал есть: между ae3-10g.mx960-1-vlgd.nwtelecom.ru и 217.107.120.117. Задержка возрастает примерно с 10 мс до 101 мс.
   - По имени первого маршрутизатора можно предположить Вологду, так как в имени есть vlgd. У второго маршрутизатора имя не отображается, поэтому его местоположение по имени определить нельзя.
