...
func main() {
	var (
		...
		verb = flag.Bool("verbose", true, "print RIP iteration log")
	)
	flag.Parse()
...
func runRIP(rs map[string]*Router, verb bool) int {
	const maxIter = 1000
			...
			for nIP, linkM := range cur.Neighbors {
				...
				for dstIP, nRoute := range nTable {
					...
					if !ok || newM < old.Metric {
						...
						if verb {
							fmt.Printf(
								"Iteration %d: router %s updated route to %s via %s, metric %d\n",
								iter,
								cur.IP,
								dstIP,
								nIP,
